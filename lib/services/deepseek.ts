...
export class DeepSeekService {
  ...
  async extractStructuredData(
    ...
    try {
      console.log(`🤖 [DEEPSEEK] Starting extraction with DeepSeek model: ${this.model}`);
      console.log(`🤖 [DEEPSEEK] Fields to extract: ${fields.map(f => f.name).join(', ')}`);
      ...
      // DeepSeek V3 has large context window but be conservative
      const MAX_CONTENT_CHARS = 300000;
      
      let trimmedContent = content;
      if (content.length > MAX_CONTENT_CHARS) {
        console.log(`[DEEPSEEK] Content too long (${content.length} chars), trimming to ${MAX_CONTENT_CHARS} chars`);
...
Example JSON structure:
{
  ...
  "fieldName_sources": [{"url": "source_url", "quote": "relevant quote"}]
      ...
      console.log(`🤖 [DEEPSEEK] Successfully received response from DeepSeek API`);
      ...
      fields.forEach(field => {
        ...
        if (value !== null && value !== undefined && confidence > 0.3) {
          results[field.name] = {
            ...
            value,
            confidence,
            source: sourcesWithQuotes ? sourcesWithQuotes.map(s => s.url).join(', ') : 'deepseek_extraction',
      ...
      throw new Error('Failed to extract structured data with DeepSeek');
    }
  }
  ...
  async generateSearchQueries(
    ...
    try {
      console.log(`🤖 [DEEPSEEK] Generating search queries for field: ${targetField}`);
      const response = await this.client.chat.completions.create({
        model: this.model,
        messages: [
          {
            role: 'system',
            content: 'Generate 3-5 targeted search queries to find specific information. Make queries specific and effective for web search.'
          },
          {
            role: 'user',
...
Generate search queries as a JSON array of strings.`
      ...
      console.error('DeepSeek query generation error:', error);
      return [];
    }
  }
} 