...
import { LLMSwitcher, type LLMProvider } from "@/components/llm-switcher";
...
import { getCurrentLLMSelection } from "@/lib/llm-manager";
...
export default function HomePage() {
  ...
  const [currentLLMProvider, setCurrentLLMProvider] = useState<LLMProvider>('grok');
  const [currentLLMModel, setCurrentLLMModel] = useState<string>('grok-3-mini');

  // Check environment status on component mount
  useEffect(() => {
    const checkEnvironment = async () => {
      try {
        // Load current LLM selection
        const llmSelection = getCurrentLLMSelection();
        setCurrentLLMProvider(llmSelection.provider as LLMProvider);
        setCurrentLLMModel(llmSelection.model);
      } catch (error) {
  ...
  const handleCSVUpload = async (rows: CSVRow[], columns: string[]) => {
    try {
      // Check if we have required API keys using the new system
      const hasRequired = await hasRequiredApiKeys();

      if (!hasRequired) {
        // Save the CSV data temporarily and show settings modal
        setPendingCSVData({ rows, columns });
        setShowSettingsModal(true);

        const missingKeys = await getMissingRequiredKeys();
        toast.error(`Missing required API keys: ${missingKeys.join(', ')}`);
      } else {
      ...
      toast.error('Failed to check API keys. Please try again.');
  ...
  const handleLLMChange = (provider: LLMProvider, modelId: string) => {
  ...
  const handleSettingsSave = () => {
    // Process pending CSV data if available
    if (pendingCSVData) {
    ...
    <div className="px-4 sm:px-6 lg:px-8 py-4 max-w-7xl mx-auto font-inter">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-3">
          <LLMSwitcher onModelChange={handleLLMChange} onNeedApiKey={openApiKeyManager} />
          <Button
